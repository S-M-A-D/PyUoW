...
`Model[ID]` extends `AuditedEntity`, `SoftDeletableEntity`, and `VersionedEntity`. Construct one without an id and it auto-generates one (via `_generate_id()`) and emits a `ModelCreatedEvent`. Calling `.delete()` emits a `ModelDeletedEvent`.
...
There are three ways to get a `Model`, and they mean different things:
...
The plain constructor reads a supplied id as *rehydration* — the entity already exists, so no creation event is emitted and `Batch.add()` rejects it with `CannotAddExistingEntityError`.

Use the `new()` classmethod when the id comes from outside — an upstream service, an idempotency key, a deterministic hash — but the entity itself is genuinely new:
...
batch.add(order)   # accepted
```

Every other keyword goes straight to the constructor, so defaults, `created_date`, and field validation behave exactly as they do for `Order(...)`. Omit `id` and `new()` is equivalent to the plain constructor.
